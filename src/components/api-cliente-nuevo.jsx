...
async function nuevoCliente(formData) {
    ...
    const response = await fetch('http://localhost:4000/clientes', {
        method: 'POST',
        body: JSON.stringify({ nombre, domicilio, fecha_nacimiento: fecha_nacimiento ? new Date(fecha_nacimiento).toISOString() : undefined, createdAt: new Date().toISOString() })
    })
...
function ClienteNew() {
    return (
        <form className='my-10 grid grid-cols-[150px_auto] gap-4'>
            ...
            <input id='domicilio' name='domicilio' className='p-1 border border-slate-200 focus:outline-blue-300 text-lg' />

            <label htmlFor='fecha_nacimiento'>Fecha de nacimiento: </label>
            <input required id='fecha_nacimiento' name='fecha_nacimiento' type='date' step='0.01' className='p-1 border border-slate-200 focus:outline-blue-300 text-lg' />
